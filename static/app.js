...
async function loadBudgets() {
    const budgetData = await api.get("/api/budgets/status");
    ...
    grid.innerHTML = budgetData.map((b) => {
        const clampPct = Math.min(b.percentage, 100);
        return `
            <div class="budget-card">
                <div class="budget-card-header">
                    <div class="budget-card-title">
                        <span class="category-dot" style="background:${esc(b.category_color)}"></span>
                        ${esc(b.category_name)}
                    </div>
                ...
                <div class="budget-progress">
                    <div class="budget-progress-fill" style="width: ${clampPct}%; background: ${b.bar_color};"></div>
                </div>
                <div class="budget-card-amounts">
                    <span>${fmt(b.spent)} spent</span>
                    <span>${fmt(b.monthly_limit)} limit &middot; ${b.percentage}%</span>
                </div>
            </div>`;
    }).join("");
}
