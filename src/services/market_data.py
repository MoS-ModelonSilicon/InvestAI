...
def fetch_batch(
    ...
    If cached_only=True, skip uncached symbols (don't fetch from network).
    If include_stale=True, also return expired cache entries (useful for
    building the screener snapshot after a long warm cycle where earlier
    entries may have expired past CACHE_TTL).
    If full=False, skip the expensive get_metrics() call for uncached
    symbols (saves ~300ms per symbol on Finnhub).
    """
    cached_results = []
    uncached = []
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        futures = {pool.submit(fetch_stock_info, sym, full): sym for sym in uncached}
...
def get_cached_quotes(symbols: list[str]) -> dict[str, dict]:
    ...
    1 000 symbols.  Used by the heatmap which must render quickly and can
    tolerate data that is up to CACHE_TTL seconds stale.

    Uses the stale-tolerant cache reader so that symbols warmed early in
    a long warm cycle are still returned even after TTL expiry (the warmer
    takes ~23 min for the full 1 005-symbol universe while CACHE_TTL is
    only 20 min).
    """
